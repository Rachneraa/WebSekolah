...
if (isset($_POST['import_guru'])) {
    ...
    // Periksa apakah library PhpSpreadsheet tersedia
    if (!class_exists(\PhpOffice\PhpSpreadsheet\IOFactory::class)) {
        $_SESSION['error'] = "Library PhpSpreadsheet tidak ditemukan. Jalankan: composer require phpoffice/phpspreadsheet";
        header("Location: ?page=guru");
        exit();
    }

    // Jika ZipArchive tidak tersedia, beri opsi fallback: terima CSV saja
    if (!class_exists('ZipArchive')) {
        // Jika user mengupload CSV, proses CSV. Jika bukan CSV, tampilkan pesan.
        if (isset($_FILES['import_file']) && $_FILES['import_file']['error'] == 0) {
            $file_name = $_FILES['import_file']['name'];
            $file_ext = strtolower(pathinfo($file_name, PATHINFO_EXTENSION));
            if ($file_ext === 'csv') {
                // Proses CSV (fallback) -- baca baris demi baris dan gunakan logika import yang sama
                ...
                if (($handle = fopen($file_tmp, 'r')) !== false) {
                    ...
                    while (($data = fgetcsv($handle)) !== false) {
                        $rownum++;
                        if ($rownum == 1) continue; // asumsi header
                        // pastikan minimal kolom sesuai: NIP, Nama, Jenis, Alamat, NoTelp, MataPelajaran
                    ...
                    fclose($handle);
                    $_SESSION['success'] = "Import CSV selesai. $imported data berhasil diimport.";
                } else {
                ...
                $_SESSION['error'] = 'Ekstensi Zip tidak aktif. Unggah file CSV atau aktifkan extension=zip di php.ini.';
                header("Location: ?page=guru");
                exit();
            }
        } else {
            $_SESSION['error'] = 'Ekstensi Zip tidak aktif. Untuk import Excel (.xlsx) aktifkan extension=zip atau unggah file CSV.';
            header("Location: ?page=guru");
            exit();
        }
    }

    // Jika ZipArchive tersedia, lanjutkan proses Excel seperti sebelumnya
    if (isset($_FILES['import_file']) && $_FILES['import_file']['error'] == 0) {
        $file_tmp = $_FILES['import_file']['tmp_name'];
        $file_name = $_FILES['import_file']['name'];
        $file_ext = strtolower(pathinfo($file_name, PATHINFO_EXTENSION));
        if (!in_array($file_ext, ['xlsx','xls','ods'])) {
            $_SESSION['error'] = "Hanya file Excel (.xlsx/.xls/.ods) yang diperbolehkan";
        } else {
            try {
...
// EXPORT: fallback ke CSV jika ZipArchive tidak tersedia
if (isset($_POST['export_guru']) || (isset($_GET['export_guru']) && !empty($_GET['export_guru']))) {
    // Jika ZipArchive tidak aktif, export CSV agar kompatibel
    if (!class_exists('ZipArchive')) {
        $filename = "data_guru_" . date("YmdHis") . ".csv";
        header('Content-Type: text/csv; charset=utf-8');
        header('Content-Disposition: attachment; filename="' . $filename . '"');
        $out = fopen('php://output', 'w');
        fputcsv($out, ["NIP", "Nama", "Jenis Kelamin", "Alamat", "No. Telepon", "Mata Pelajaran"]);
    ...
    // Jika ZipArchive tersedia, lanjutkan export Excel/PhpSpreadsheet seperti semula
    // ... existing Excel export code here ...
}
