            ...
            <form method="post" action="admin.php?page=jadwal" class="row g-2">
                ...
                <div class="col-md-2">
                    <select name="kelas_id" class="form-select" required>
                        ...
                        <?php mysqli_data_seek($kelas, 0);
                        while ($k = mysqli_fetch_assoc($kelas)):
                            // safe access untuk menghindari notice/warning
                            $kelas_id_val = isset($k['kelas_id']) ? $k['kelas_id'] : (isset($k['id']) ? $k['id'] : '');
                            $kelas_name = isset($k['nama']) ? $k['nama'] : '';
                            $selected_kelas = (($edit_data && isset($edit_data['kelas_id']) && $edit_data['kelas_id'] == $kelas_id_val) || ($filter_kelas == $kelas_id_val)) ? 'selected' : '';
                        ?>
                            <option value="<?= htmlspecialchars($kelas_id_val) ?>" <?= $selected_kelas ?>>
                                <?= htmlspecialchars($kelas_name) ?>
                            </option>
                        <?php endwhile; ?>
                    </select>
                </div>
                ...
                <div class="col-md-3">
                    <select name="mapel_id" class="form-select" required>
                        ...
                        while ($m = mysqli_fetch_assoc($mapel)):
                            // safe access untuk menghindari notice/warning
                            $mapel_id_val = isset($m['id']) ? $m['id'] : (isset($m['mapel_id']) ? $m['mapel_id'] : '');
                            $mapel_name = isset($m['nama']) ? $m['nama'] : (isset($m['nama_mapel']) ? $m['nama_mapel'] : '');
                            $selected_mapel = ($edit_data && isset($edit_data['mapel_id']) && $edit_data['mapel_id'] == $mapel_id_val) ? 'selected' : '';
                        ?>
                            <option value="<?= htmlspecialchars($mapel_id_val) ?>" <?= $selected_mapel ?>>
                ...
                <div class="col-md-2">
                    <select name="guru_id" class="form-select" required>
                        ...
                        while ($g = mysqli_fetch_assoc($guru)):
                            // safe access untuk menghindari notice/warning
                            $guru_id_val = isset($g['guru_id']) ? $g['guru_id'] : (isset($g['id']) ? $g['id'] : '');
                            $guru_name = isset($g['nama']) ? $g['nama'] : '';
                            $selected_guru = ($edit_data && isset($edit_data['guru_id']) && $edit_data['guru_id'] == $guru_id_val) ? 'selected' : '';
                        ?>
                            <option value="<?= htmlspecialchars($guru_id_val) ?>" <?= $selected_guru ?>>
                                <?= htmlspecialchars($guru_name) ?>
                            </option>
                        <?php endwhile; ?>
                    </select>
                </div>
