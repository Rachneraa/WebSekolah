...
function generateQRCode($kelas_id, $nama_kelas)
{
    try {
        ...
        // Bangun URL absensi yang valid (arahkan ke folder modules)
        $scheme = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
        $host = $scheme . '://' . $_SERVER['HTTP_HOST'];
        // ambil base path (mis. /smk-ti-main/backend)
        $basePath = rtrim(dirname($_SERVER['SCRIPT_NAME']), '/\\');
        $qr_content = $host . $basePath . '/modules/absensi.php?kelas=' . $kelas_id;
...
{
    // Ambil kolom yang diperlukan dan GROUP BY semua kolom non-aggregat agar kompatibel dengan ONLY_FULL_GROUP_BY
    // Gunakan kolom asli: kelas_id (bukan id)
                            ...
                            while ($row = $kelas_list->fetch_assoc()):
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <br>
                                        <a href="modules/absensi.php?kelas=<?= $row['kelas_id'] ?>"
